...
class GATv2Net(torch.nn.Module):
    def __init__(self, node_feature_dim, edge_feature_dim, config, graph_level_dim=1):
        ...
        # MLPs — fc1 input includes pooled GNN output + graph-level Coulomb energy
        self.fc1 = nn.Linear(final_dim * 2 + graph_level_dim, 1024)
    ...
    def forward(self, data):
        ...
        coulomb_energy = data.coulomb_energy  # shape: [B]
        ...
        # Pool + concatenate graph-level Coulomb energy
        x = torch.cat([gmp(x, batch), gap(x, batch), coulomb_energy.view(-1, 1)], dim=1)
