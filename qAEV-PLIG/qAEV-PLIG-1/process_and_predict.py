...
from helpers import (
    model_dict, load_charges, get_heavy_atom_charges,
    get_protein_coords_and_charges,
    compute_coulomb_potential, compute_coulomb_energy,
)
from config import EPSILON_R, COULOMB_CUTOFF, ELEMENT_LIST
import argparse
import time
import sys
import warnings
...
def mol_to_graph(mol, mol_df, aevs, heavy_atom_charges, coulomb_potentials, coulomb_energy):
    """Build graph with Coulomb features. Returns 5-element tuple."""
    features = []
    heavy_atom_index = []
    idx_to_idx = {}
    counter = 0
    ...
    for atom in mol.GetAtoms():
        if atom.GetSymbol() != "H":
            ...
            base_feat = atom_features(atom)
            aev_feat = aevs[aev_idx, :].numpy().flatten()
            charge_feat = np.array([heavy_atom_charges[counter]])
            potential_feat = np.array([coulomb_potentials[counter]])
            feature = np.concatenate([base_feat, aev_feat, charge_feat, potential_feat])
...
def process_single_graph(row_dict, atom_keys, radial_coefs, atom_map):
    ...
    charges_json_file = row_dict["charges_json"]
    ...
    # Load charges and compute Coulomb features
    charges = load_charges(charges_json_file)
    heavy_charges = get_heavy_atom_charges(lig, charges)
    prot_coords, prot_charges = get_protein_coords_and_charges(pdb_file, charges)

    lig_coords = []
    for atom in lig.GetAtoms():
        if atom.GetSymbol() != "H":
            pos = lig.GetConformer().GetAtomPosition(atom.GetIdx())
            lig_coords.append([pos.x, pos.y, pos.z])
    lig_coords = np.array(lig_coords)

    potentials = compute_coulomb_potential(lig_coords, prot_coords, prot_charges,
                                           epsilon_r=EPSILON_R, cutoff=COULOMB_CUTOFF)
    energy = compute_coulomb_energy(lig_coords, np.array(heavy_charges), prot_coords, prot_charges,
                                    epsilon_r=EPSILON_R, cutoff=COULOMB_CUTOFF)

    # AEVs
    mol_df, aevs = GetMolAEVs_extended(pdb_file, lig, atom_keys, radial_coefs, atom_map)

    graph = mol_to_graph(lig, mol_df, aevs, heavy_charges, potentials, energy)
...
def make_predictions(config):
    ...
    # Handle both old format (just y_scaler) and new format (dict with both scalers)
    if isinstance(scalers, dict):
        scaler = scalers['y_scaler']
        coulomb_scaler = scalers['coulomb_scaler']
    else:
        scaler = scalers
        coulomb_scaler = None
    ...
    # Apply coulomb_scaler to graph-level energies before creating the dataset
    if coulomb_scaler is not None:
        for uid, graph in graphs_dict.items():
            c_size, features, edge_index, edge_attr, coulomb_energy = graph
            scaled_energy = coulomb_scaler.transform([[coulomb_energy]])[0, 0]
            graphs_dict[uid] = (c_size, features, edge_index, edge_attr, scaled_energy)
