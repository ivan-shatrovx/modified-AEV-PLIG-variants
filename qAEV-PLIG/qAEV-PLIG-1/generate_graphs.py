...
Reads dataset.csv, generates per-complex graphs with:
  ...
  - Two new node features: partial charge, Coulomb potential
  - One graph-level scalar: Coulomb interaction energy

Outputs a pickle dict: {unique_id: (c_size, features, edge_index, edge_attr, coulomb_energy)}
"""
...
from config import (
    ...
    ELEMENT_LIST, EPSILON_R, COULOMB_CUTOFF,
)
from helpers import (
...
# ═══════════════════════════════════════════════════════════════════
# Functions reused from original generate_pdbbind_graphs.py
# ═══════════════════════════════════════════════════════════════════
...
if __name__ == "__main__":
    ...
    for i, row in tqdm(data.iterrows(), total=len(data)):
        ...
        try:
            ...
            # Protein coords + charges (ALL atoms for Coulomb)
            ...
            # Coulomb features
            potentials = compute_coulomb_potential(
                lig_coords, prot_coords, prot_charges,
                epsilon_r=EPSILON_R, cutoff=COULOMB_CUTOFF)
            energy = compute_coulomb_energy(
                lig_coords, np.array(heavy_charges), prot_coords, prot_charges,
                epsilon_r=EPSILON_R, cutoff=COULOMB_CUTOFF)

            # AEVs (uses only protein heavy atoms, via LoadPDBasDF)
