...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='pdbbind_U_bindingnet_U_bindingdb_ligsim90_fep_benchmark_dpk6')
...
def train_NN(args):
    ...
    print('Running dataset {} with global_feat_dim={}.'.format(dataset, GLOBAL_FEAT_DIM))
    ...
    for i, seed in enumerate(seeds):
        ...
        model_file_name = timestr + '_model_GATv2Net_' + dataset + '_' + str(i) + '.model'
        ...
        print("Node features:", train_data.num_node_features)
        print("Edge features:", train_data.num_edge_features)
        ...
        df_test['preds_' + str(i)] = P_test

    df_test['preds'] = df_test.iloc[:, 1:].mean(axis=1)

    scaler_file = timestr + '_model_GATv2Net_' + dataset + '.pickle'
    with open(os.path.join(model_output_dir, scaler_file), 'wb') as f:
        pickle.dump(train_data.y_scaler, f)

    test_preds = np.array(df_test['preds'])
    test_truth = np.array(df_test['truth'])
    print("Ensemble test PC:", pearson(test_truth, test_preds))
    print("Ensemble test RMSE:", rmse(test_truth, test_preds))
