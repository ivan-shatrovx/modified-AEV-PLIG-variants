...
from model_defs_dpk6 import GATv2Net
import argparse
import time
import sys
import warnings
...
GLOBAL_FEAT_DIM = 6  # 5 direct + 1 aromatic sum
...
def make_predictions(config):
    ...
    """
    Build global features dict — 5 direct DPK columns + aromatic sum
    """
    # Accept either pre-summed dpk_aromatic or individual PHE/HIS/TYR/TRP columns
    if 'dpk_aromatic' in data.columns and not all(c in data.columns for c in DPK_AROMATIC_COLS):
        all_cols = DPK_DIRECT_COLS + ['dpk_aromatic']
    else:
        all_cols = DPK_DIRECT_COLS + DPK_AROMATIC_COLS
        data['dpk_aromatic'] = data[DPK_AROMATIC_COLS].sum(axis=1)

    nan_rows = data[all_cols].isna().any(axis=1)
    if nan_rows.any():
        print(f"WARNING: {nan_rows.sum()} row(s) with NaN global features — imputing with column median.")
        for col in all_cols:
            data[col] = data[col].fillna(data[col].median())

    output_cols = DPK_DIRECT_COLS + ['dpk_aromatic']

    global_dict = {
        row['unique_id']: np.array(row[output_cols], dtype=np.float32)
    ...
    test_data = GraphDatasetPredict(root='data', dataset=config.data_name, ids=test_ids, graph_ids=test_graph_ids, graphs_dict=graphs_dict, global_dict=global_dict)
    ...
    model = GATv2Net(node_feature_dim=test_data.num_node_features, edge_feature_dim=test_data.num_edge_features, config=config, global_feat_dim=GLOBAL_FEAT_DIM)
