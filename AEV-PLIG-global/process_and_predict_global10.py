...
def make_predictions(config):
    ...
    """
    Build global features dict
    """
    global_feat_cols = config.global_feature_cols
    print(f"Using global features: {global_feat_cols}\n")
    nan_rows = data[global_feat_cols].isna().any(axis=1)
    if nan_rows.any():
    ...
    global_dict = {
        row['unique_id']: np.array(row[global_feat_cols], dtype=np.float32)
        for _, row in data.iterrows()
    }
    global_feat_dim = len(global_feat_cols)
    ...
    model = modeling(node_feature_dim=test_data.num_node_features, edge_feature_dim=test_data.num_edge_features, config=config, global_feat_dim=global_feat_dim)
...
def parse_args():
    ...
    parser.add_argument('--global_feature_cols', type=str, nargs='+',
                        default=['arp_proximal', 'rdk_MolLogP', 'arp_hbond', 'arp_hydrophobic',
                                 'rdk_fr_halogen', 'rdk_RingCount', 'dpk_volume_score',
                                 'rdk_TPSA', 'dpk_prop_polar_atm', 'dpk_mean_loc_hyd_dens'],
                        help='Column names in the CSV to use as global features.')
