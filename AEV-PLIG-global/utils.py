...
class GraphDataset(InMemoryDataset):
    def __init__(self, root='data', dataset=None,
                 ids=None, y=None, graphs_dict=None, y_scaler=None, global_dict=None):
        ...
        if os.path.isfile(self.processed_paths[0]):
            ...
            self.process(ids, y, graphs_dict, global_dict)
            #self.data, self.slices = torch.load(self.processed_paths[0])
            self.load(self.processed_paths[0])
    ...
    def process(self, ids, y, graphs_dict, global_dict):
        ...
        for i in range(data_len):
            ...
            if pdbcode not in global_dict:
                raise KeyError(f"unique_id '{pdbcode}' not found in global features dict")
            data_point = Data(x=torch.Tensor(np.array(features)),
                                   ...
                                   global_features=torch.tensor(global_dict[pdbcode], dtype=torch.float))
...
class GraphDatasetPredict(InMemoryDataset):
    def __init__(self, root='data', dataset=None,
                 ids=None, graph_ids=None, graphs_dict=None, global_dict=None):
        ...
        if os.path.isfile(self.processed_paths[0]):
            ...
            self.process(ids, graph_ids, graphs_dict, global_dict)
            self.load(self.processed_paths[0])
    ...
    def process(self, ids, graph_ids, graphs_dict, global_dict=None):
        ...
        for i in range(data_len):
            ...
            if global_dict is not None:
                if pdbcode not in global_dict:
                    raise KeyError(f"unique_id '{pdbcode}' not found in global features dict")
                kwargs['global_features'] = torch.tensor(global_dict[pdbcode], dtype=torch.float)
            data_point = Data(x=torch.Tensor(np.array(features)),
                                   edge_index=torch.LongTensor(np.array(edge_index)).T,
                                   edge_attr=torch.Tensor(np.array(edge_features)),
