...
class GATv2Net(torch.nn.Module):
    def __init__(self, node_feature_dim, edge_feature_dim, config, global_feat_dim=10):
        ...
        # MLPs
        self.fc1 = nn.Linear(final_dim * 2 + global_feat_dim, 1024)
    ...
    def forward(self, data):
        ...
        global_vec = data.global_features.view(-1, self.global_feat_dim)
        global_proj = self.global_mlp(global_vec)
        x = torch.cat([gmp(x, batch), gap(x, batch), global_proj], dim=1)
